...
class ProbabilityEngine {
    ...
    _buildIndex() {
        ...
        for (const [cat, records] of Object.entries(this.historyByCategory)) {
            ...
            this.avgRateByCategory[cat] = rates.length ? rates.reduce((a, b) => a + b, 0) / rates.length : 50;
        }
    }
    ...
    calculate(subsidy, profile) {
        ...
        const final = this._integrate(eligibility, matching, competition, historical, subsidy);
    ...
    _chkRegion(sub, prof) {
        ...
        if (!req || ['전국','해당없음','-',''].includes(req)) return 100;
    ...
    _chkIndustry(sub, prof) {
        ...
        if (!req || ['전체','전업종','해당없음','-',''].includes(req)) return 100;
        ...
        if (uCats.some(c => rCats.includes(c))) return 90;
        if (text.includes('소상공인') || text.includes('소기업')) return 80;
        return 50;
    }
    ...
    _chkBizType(sub, prof) {
        ...
        if (!user) return 50;
        if (text.includes('소상공인')) return 85;
    ...
    _chkRevenue(sub, prof) {
        ...
        if (!sub.revenue_range || ['해당없음','-',''].includes(sub.revenue_range)) return 85;
        if (rev == null) return 50;
    ...
    _chkEmployee(sub, prof) {
        ...
        if (!sub.employee_range || ['해당없음','-',''].includes(sub.employee_range)) return 85;
        if (cnt == null) return 50;
    ...
    _chkAge(sub, prof) {
        ...
        if (text.includes('예비창업')) return age < 0.5 ? 100 : age < 1 ? 50 : 10;
        if (text.includes('초기창업') || text.includes('3년 이내') || text.includes('3년이내'))
            return age <= 3 ? 100 : age <= 5 ? 50 : 20;
        if (text.includes('7년 이내') || text.includes('7년이내'))
            return age <= 7 ? 100 : age <= 10 ? 60 : 30;
        return 80;
    }
    ...
    _chkSpecial(sub, prof) {
        ...
        if (text.includes('여성') && prof.is_female_owned) score += 15;
        if (text.includes('장애인') && prof.is_disabled_owned) score += 15;
        if (text.includes('사회적기업') && prof.is_social_enterprise) score += 10;
        if ((text.includes('벤처') || text.includes('혁신')) && prof.is_venture_certified) score += 10;
        if (text.includes('이노비즈') && prof.is_innobiz) score += 10;
        if (text.includes('수출') && prof.has_export) score += 10;
        if (text.includes('고용보험') && !prof.has_employment_insurance) score -= 20;
        if ((prof.previous_subsidy_count || 0) > 3) score -= 10;
    ...
    _calcMatching(sub, prof) {
        ...
        const weights = { industry_match: 0.25, scale_match: 0.25, purpose_match: 0.20, region_match: 0.15, qualification_match: 0.15 };
    ...
    _matchIndustry(text, prof) {
        ...
        if (uC.some(c => tC.includes(c))) return 85;
        if (text.includes('전업종') || text.includes('업종무관') || text.includes('소상공인')) return 75;
        return 40;
    }

    _matchScale(text, prof) {
        const rev = prof.annual_revenue || 0, emp = prof.employee_count || 0;
        if (text.includes('소상공인')) return (rev <= 1e9 && emp < 5) ? 95 : (rev <= 5e9 && emp < 10) ? 80 : 50;
        if (text.includes('소기업')) return (rev <= 5e9 && emp < 50) ? 90 : 60;
        if (text.includes('중소기업')) return rev <= 30e9 ? 85 : 50;
        return 70;
    }
    ...
    _matchRegion(text, prof) {
        const user = prof.region_sido || '';
        if (!user) return 50;
        if (text.includes('전국') || !REGIONS.some(r => text.includes(r))) return 80;
        return REGIONS.some(r => text.includes(r) && user.includes(r)) ? 100 : 40;
    }

    _matchPurpose(sub, prof) {
        const cat = sub.category || '';
        const age = prof.business_age_years;
        if (cat === '창업') return age != null && age <= 3 ? 90 : age != null && age <= 7 ? 60 : 30;
        if (cat === '수출') return prof.has_export ? 90 : 40;
        if (cat === '기술') {
            const techKws = ['IT', '소프트웨어', '바이오', '제조'];
            if (techKws.some(k => (prof.industry_name||'').includes(k))) return 85;
            if (prof.is_venture_certified || prof.is_innobiz) return 80;
            return 50;
        }
        if (cat === '금융') return 75;
        if (cat === '인력') return (prof.employee_count || 0) >= 1 ? 80 : 50;
        if (cat === '경영') return 75;
        return 60;
    }
    ...
    _matchQual(text, prof) {
        let score = 70;
        if (prof.is_venture_certified && text.includes('벤처')) score += 10;
        if (prof.is_social_enterprise && text.includes('사회적')) score += 10;
        if (prof.technology_grade && (text.includes('기술') || text.includes('R&D'))) score += 10;
    ...
    _calcCompetition(sub) {
        ...
        const rate = this.avgRateByCategory[cat] || 50;
        const score = 100 / (1 + Math.exp(-0.05 * (rate - 50)));
        return { score, selection_rate: Math.round(rate * 10) / 10 };
    }
    ...
    _calcHistorical(sub, prof) {
        ...
        if (!records.length) return { score: 50 };

        const factors = {};
        const age = prof.business_age_years;
        const avgAge = records.reduce((s, r) => s + (r.avg_company_age || 5), 0) / records.length;
        factors.age_fit = age != null ? Math.max(0, 100 - Math.abs(age - avgAge) * 10) : 50;

        const emp = prof.employee_count;
        const avgEmp = records.reduce((s, r) => s + (r.avg_employee_count || 5), 0) / records.length;
        factors.employee_fit = emp != null ? Math.max(0, 100 - Math.abs(1 - (emp / (avgEmp || 1))) * 50) : 50;

        const rev = prof.annual_revenue;
        const avgRev = records.reduce((s, r) => s + (r.avg_revenue || 0), 0) / records.length;
        factors.revenue_fit = (rev && avgRev) ? Math.max(0, 100 - Math.abs(1 - (rev / avgRev)) * 30) : 50;
        ...
        factors.industry_fit = Math.min(100, (indMatch / Math.max(1, records.length)) * 100 + 30);
        ...
        factors.region_fit = Math.min(100, (regMatch / Math.max(1, records.length)) * 100 + 20);

        const weights = { age_fit: 0.15, employee_fit: 0.15, revenue_fit: 0.20, industry_fit: 0.30, region_fit: 0.20 };
        const score = Object.entries(weights).reduce((s, [k, w]) => s + (factors[k] || 50) * w, 0);
        return { score, factors };
    }

    // 5단계: 통합
    _integrate(elig, match, comp, hist, sub) {
        if (elig.disqualified) return Math.max(5, elig.score * 0.3);
        const cat = sub.category || '기타';
        const w = this._catWeights(cat);
        const raw = elig.score * w.eligibility + match.score * w.matching + comp.score * w.competition + hist.score * w.historical;
        const norm = (raw - 50) / 50 * 0.8;
        const adjusted = (norm + 1) * 50;
        const baseRate = this.avgRateByCategory[cat] || 50;
        const anchored = adjusted * 0.7 + baseRate * 0.3;
        return Math.max(5, Math.min(95, anchored));
    }
    ...
    _recommend(sub, prof, elig, match) {
        ...
        if (elig.disqualified) {
            ...
            for (const [name, chk] of Object.entries(elig.checks || {})) {
                if (chk.score === 0 && chk.weight >= 15 && labels[name])
                    recs.push(`⚠️ ${labels[name]} 요건이 충족되지 않아 지원이 어려울 수 있습니다.`);
            }
        }
        ...
        if (text.includes('여성') && !prof.is_female_owned) recs.push('ℹ️ 여성기업 가산점이 있는 사업입니다.');
        if (text.includes('벤처') && !prof.is_venture_certified) recs.push('ℹ️ 벤처인증 기업에 가산점이 부여됩니다.');
